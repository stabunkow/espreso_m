...
void setDistribution(InputFile &file, size_t begin, size_t end, std::vector<size_t> &tdistribution, bool toLines = false)
{
	...
	if (mpibegin < mpiend) {
		...
		if (tdistribution[0] < mpibegin) tdistribution[0] = mpibegin;
		for (int t = 0; t < info::env::OMP_NUM_THREADS; ++t) {
			if (tdistribution[t + 1] < tdistribution[t]) {
				tdistribution[t + 1] = tdistribution[t];
			}
			if (tdistribution[t + 1] > mpiend) tdistribution[t + 1] = mpiend;
		}
	} else {
