...
void VTKLegacyGeometry::parseASCII(MeshBuilder &mesh, const std::vector<std::string> &names)
{
	...
	while (_pack.next()) {
		...
		// printf("packing: rank: %d, index: %d\n", info::mpi::rank, _pack.fileindex);
		if (_points[_pack.fileindex].begin < _points[_pack.fileindex].end) {
			ASCIIParser::parse(points[_pack.fileindex], _pack, _points[_pack.fileindex].begin, _points[_pack.fileindex].end);
		} else {
			// printf("point-error: rank: %d, begin: %d, end: %d\n", info::mpi::rank, _points[_pack.fileindex].begin, _points[_pack.fileindex].end);
		}
		if (_cells[_pack.fileindex].begin < _cells[_pack.fileindex].end) {
			ASCIIParser::parse(cells[_pack.fileindex], _pack, _cells[_pack.fileindex].begin, _cells[_pack.fileindex].end);
		} else {
			// printf("cell-error: rank: %d, begin: %d, end: %d\n", info::mpi::rank, _cells[_pack.fileindex].begin, _cells[_pack.fileindex].end);
		}
		if (_cellTypes[_pack.fileindex].begin < _cellTypes[_pack.fileindex].end) {
			ASCIIParser::parse(celltypes[_pack.fileindex], _pack, _cellTypes[_pack.fileindex].begin, _cellTypes[_pack.fileindex].end);
		} else {
			// printf("cell-type error: rank: %d, begin: %d, end: %d\n", info::mpi::rank, _cellTypes[_pack.fileindex].begin, _cellTypes[_pack.fileindex].end);
		}
		// ASCIIParser::parse(points[_pack.fileindex], _pack, _points[_pack.fileindex].begin, _points[_pack.fileindex].end);
		// ASCIIParser::parse(cells[_pack.fileindex], _pack, _cells[_pack.fileindex].begin, _cells[_pack.fileindex].end);
		// ASCIIParser::parse(celltypes[_pack.fileindex], _pack, _cellTypes[_pack.fileindex].begin, _cellTypes[_pack.fileindex].end);
	...
	// printf("Parse finished: %03d, packsize: %d\n", info::mpi::rank, _pack.size());
