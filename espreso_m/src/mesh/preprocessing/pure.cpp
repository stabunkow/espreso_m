	...
	if (info::mpi::size <= info::ecf->input.third_party_scalability_limit && info::ecf->input.decomposition.parallel_decomposer != DecompositionConfiguration::ParallelDecomposer::METIS) {
		...
		case DecompositionConfiguration::ParallelDecomposer::PARMETIS:
			if (info::ecf->input.loaddomain) {
				...
				auto callDist = [] (const Point &a, const Point &b) {
					// double x = a.x - b.x;
					// double y = b.y - b.y;
					// double z = a.z - b.z;
					// return x * x + y * y + z * z;
				...
				auto isInDomain = [] (const Point &a, const Point &minp, const Point &maxp) {
					// if (a.x >= minp.x && 
					// 	a.y >= minp.y &&
					// 	a.z >= minp.z &&
					// 	a.x <= maxp.x && 
				...
				int elementSiz = partition.size();
				int domainSiz = info::mesh->loadDomainSiz;
				std::vector<esint> vertWeight(elementSiz * 2, 1);
				std::vector<float> coords(elementSiz * 3);
				#pragma omp parallel for
				for (int e = 0; e < elementSiz; ++e) {
					int bstd = -1;
					double bstdist = 1e9;
					auto point = elements->centers->datatarray()[e];
					for (int d = 0; d < domainSiz; ++d) {
						if (isInDomain(point, info::mesh->loadDomainMin[d], info::mesh->loadDomainMax[d])) {
							double dist = callDist(elements->centers->datatarray()[e], info::mesh->loadDomainCenter[d]);
							if (dist < bstdist) {
								bstd = d;
								bstdist = dist;
							}
					...
					if (bstd == -1) {
						for (int d = 0; d < domainSiz; ++d) {
							double dist = callDist(point, info::mesh->loadDomainCenter[d]);
							if (dist < bstdist) {
								bstd = d;
								bstdist = dist;
							}
					...
					coords[e * 3] = (float) point.x;
					coords[e * 3 + 1] = (float) point.y;
					coords[e * 3 + 2] = (float) point.z;
					vertWeight[e] = info::mesh->loadDomainWeight[bstd];
				}
				...
				edgecut = ParMETIS::call(ParMETIS::METHOD::ParMETIS_V3_PartKway,
					...
					3, coords.data(), 1, vertWeight.data(), NULL, partition.data());

				// Check Weight
				std::vector<int> pw(info::mpi::size);
				std::vector<int> partElemSiz(info::mpi::size);
				for (int i = 0; i < elementSiz; ++i) {
					int pid = partition[i];
					pw[pid] += vertWeight[i];
					partElemSiz[pid] += 1;
				}

				for (int p = 0; p < info::mpi::size; ++p) {
					int totalPw = 0;
					int totalElemSiz = 0;
					Communication::allReduce(&partElemSiz[p], &totalElemSiz, 1, MPI_INT, MPI_SUM);
					Communication::allReduce(&pw[p], &totalPw, 1, MPI_INT, MPI_SUM);
					if (info::mpi::rank == 0) {
						printf("Partition-%3d: %d %d\n", p, totalElemSiz, totalPw);
					}
				}
			} else {
				if (info::ecf->input.geomparmetis) {
					...
					edgecut = ParMETIS::call(ParMETIS::METHOD::ParMETIS_V3_PartKway,
						MPITools::subset->across, edistribution.data(), eframes.data(), eneighbors.data(),
						3, coords.data(), 0, NULL, NULL, partition.data());
				} else {
					edgecut = ParMETIS::call(ParMETIS::METHOD::ParMETIS_V3_PartKway,
						MPITools::subset->across, edistribution.data(), eframes.data(), eneighbors.data(),
						NULL, NULL, 0, NULL, NULL, partition.data());
				}
			}
			...
			if (info::ecf->input.scancluster) {
				...
				if (info::mpi::rank == 0) {
					...
					std::vector<std::vector<esint> > changeClusterVerts;
					std::vector<std::unordered_set<esint> > changeClusterNeighbors;
					std::vector<std::vector<esint> > rankClusterSizs(info::mpi::size);
					std::vector<bool> vertVisited(gpartition.size(), false);

					printf("Partition Scanning\n");
					
					for (esint v = 0; v < gpartition.size(); ++v) {
						if (vertVisited[v]) continue;
						esint rank = gpartition[v];
						std::queue<esint> clusterQue;
						std::unordered_set<esint> neighs;
						std::vector<esint> verts;
						clusterQue.push(v);
						verts.push_back(v);
						vertVisited[v] = true;
						while (!clusterQue.empty()) {
							esint now = clusterQue.front();
							clusterQue.pop();
							for (esint edgetex = gframes[now]; edgetex < gframes[now + 1]; ++edgetex) {
								esint next = gneighbors[edgetex];
								if (next < 0 || next >= gpartition.size()) continue; // sometime error
								esint npart = gpartition[next];
								if (npart == rank) {
									if (!vertVisited[next]) {
										clusterQue.push(next);
										verts.push_back(next);
										vertVisited[next] = true;
									}
								} else {
									neighs.insert(npart);
								}
						...
						rankClusterSizs[rank].push_back(verts.size());
						if (verts.size() <= 100) {
							changeClusterVerts.push_back(verts);
							changeClusterNeighbors.push_back(neighs);
						}
					}
					for (esint rank = 0; rank < info::mpi::size; ++rank) {
						printf("rank-%04d: clusterSiz: %d, : ", rank, rankClusterSizs[rank].size());
						for (esint i = 0; i < rankClusterSizs[rank].size(); ++i) {
							printf("  %d,", rankClusterSizs[rank][i]);
						}
						printf("\n");
					}	
					for (esint i = 0; i < changeClusterVerts.size(); ++i) {
						auto neigh = changeClusterNeighbors[i].begin();
						for (esint j = 0; j < changeClusterVerts[i].size(); ++j) {
							gpartition[changeClusterVerts[i][j]] = *neigh;
						}
					}
				}
