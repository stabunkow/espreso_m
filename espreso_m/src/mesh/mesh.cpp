...
void Mesh::preprocess()
{
	...
	if (info::ecf->input.loaddomain) {
		if (info::mpi::rank == 0) {
			FILE * fp;
			printf("loading\n");
			fp = fopen("./loaddomain.dat", "r");
			fscanf(fp, "%d", &loadDomainSiz);
			std::vector<double> dwgt(loadDomainSiz);
			loadDomainWeight.resize(loadDomainSiz);
			loadDomainCenter.resize(loadDomainSiz);
			loadDomainMin.resize(loadDomainSiz);
			loadDomainMax.resize(loadDomainSiz);
			for (esint d = 0; d < loadDomainSiz; ++d) {
				fscanf(fp, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf", 
					   &dwgt[d], 
					   &loadDomainCenter[d].x, &loadDomainCenter[d].y, &loadDomainCenter[d].z,
					   &loadDomainMin[d].x, &loadDomainMin[d].y, &loadDomainMin[d].z,
					   &loadDomainMax[d].x, &loadDomainMax[d].y, &loadDomainMax[d].z
				);
			}
			fclose(fp);

			double minwgt = *std::min_element(dwgt.begin(), dwgt.end());
			int scale = (int) (1. / minwgt);
			if (scale < 1) scale = 1;
			else scale = scale + 1;
			for (esint d = 0; d < loadDomainSiz; ++d) dwgt[d] = dwgt[d] * scale;
			// while (minwgt < 2) {
			// 	minwgt *= 2;
			// 	for (esint d = 0; d < loadDomainSiz; ++d) dwgt[d] = dwgt[d] * 2;
			// }
			for (esint d = 0; d < loadDomainSiz; ++d) loadDomainWeight[d] = (esint) dwgt[d];
			for (esint d = 0; d < loadDomainSiz; ++d) {
				printf("%.2f-%.2f: %d\n", loadDomainMin[d].z, loadDomainMax[d].z, loadDomainWeight[d]);
			}
			printf("loadover\n");

			Communication::broadcast(&loadDomainSiz, 1, MPI_INT, 0);
			Communication::broadcast(&loadDomainWeight[0], loadDomainSiz, esint_mpi, 0);
			Communication::broadcast(&loadDomainCenter[0], loadDomainSiz * sizeof(Point), MPI_BYTE, 0);
			Communication::broadcast(&loadDomainMin[0], loadDomainSiz * sizeof(Point), MPI_BYTE, 0);
			Communication::broadcast(&loadDomainMax[0], loadDomainSiz * sizeof(Point), MPI_BYTE, 0);
		} else {
			Communication::broadcast(&loadDomainSiz, 1, MPI_INT, 0);
			loadDomainWeight.resize(loadDomainSiz);
			loadDomainCenter.resize(loadDomainSiz);
	...
	if (info::ecf->input.geomparmetis) {
		mesh::computeElementsCenters(nodes, elements);
	}
	reclusterize();
	...
	if (info::ecf->input.loaddomain) {
		int avgdomain = preferedDomains;
		int localElementSiz = elements->distribution.process.size;
		int totalElementSiz = 0;
		Communication::allReduce(&localElementSiz, &totalElementSiz, 1, MPI_INT, MPI_SUM);
		int avgElementSiz = totalElementSiz / info::mpi::size;
		double ratio = localElementSiz * 1. / avgElementSiz;
		int tgtdomain = avgdomain * ratio;
		preferedDomains = tgtdomain;
		// printf("tgtdomain: %d\n", (int) preferedDomains);
	}
...
void Mesh::printDecompositionStatistics()
{
	...
	double lscSiz = 0;
	int csiz = clusters->size;
	std::vector<double> clsc(csiz);
	for (esint d = 0; d < domains->size; ++d) {
		double w = dninterface[d] * dninterface[d] * sizeof(double) * 1. / (1024 * 1024 * 1024);
		int c = domains->cluster[d];
		clsc[c] += w;
		lscSiz += w;
	}
	info::mpi::printStatsMPI("Estimated LSC", lscSiz);

	if (info::ecf->input.savedomain) {
		mesh::computeElementsCenters(nodes, elements);
		std::vector<Point> cmin(clusters->size, Point(1e9)), cmax(clusters->size, Point(-1e9));
		#pragma omp parallel for
		for (esint d = 0; d < domains->size; ++d) {
			Point dmin(1e9), dmax(-1e9);
			for (esint e = domains->elements[d]; e < domains->elements[d + 1]; ++e) {
				auto point = elements->centers->datatarray()[e];
				if (point.x < dmin.x) dmin.x = point.x;
				if (point.y < dmin.y) dmin.y = point.y;
				if (point.z < dmin.z) dmin.z = point.z;
				if (point.x > dmax.x) dmax.x = point.x;
				if (point.y > dmax.y) dmax.y = point.y;
				if (point.z > dmax.z) dmax.z = point.z;
			}
			int c = domains->cluster[d];
			#pragma omp critical 
			{
				if (dmin.x < cmin[c].x) cmin[c].x = dmin.x;
				if (dmin.y < cmin[c].y) cmin[c].y = dmin.y;
				if (dmin.z < cmin[c].z) cmin[c].z = dmin.z;
				if (dmax.x > cmax[c].x) cmax[c].x = dmax.x;
				if (dmax.y > cmax[c].y) cmax[c].y = dmax.y;
				if (dmax.z > cmax[c].z) cmax[c].z = dmax.z;
			}
		}

		std::vector<double> geoWgt(csiz);
		std::vector<double> geoMin(3 * csiz, 1e9), geoMax(3 * csiz, -1e9);
		if (csiz == 1) cdomains[0] = domains->size;
		for (esint c = 0; c < csiz; ++c) {
			int x = 3 * c;
			int y = 3 * c + 1;
			int z = 3 * c + 2;
			geoMin[x] = cmin[c].x;
			...
			geoMax[y] = cmax[c].y;
			geoMax[z] = cmax[c].z;
			geoWgt[c] = clsc[c] / cdomains[c];
		}
		
		std::vector<double> geoMins, geoMaxs, geoWgts;
		Communication::gatherUnknownSize(geoMin, geoMins);
		Communication::gatherUnknownSize(geoMax, geoMaxs);
		Communication::gatherUnknownSize(geoWgt, geoWgts);
		if (info::mpi::rank == 0) {
			FILE * fp;
			fp = fopen("./savedomain.dat", "w+");
			fprintf(fp, "%d\n", geoWgts.size());
			for (esint c = 0; c < geoWgts.size(); ++c) {
				int x = 3 * c;
				int y = 3 * c + 1;
				int z = 3 * c + 2;
				double cetX = (geoMins[x] + geoMaxs[x]) / 2;
				double cetY = (geoMins[y] + geoMaxs[y]) / 2;
				double cetZ = (geoMins[z] + geoMaxs[z]) / 2;
				fprintf(fp, "%.6f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f\n", 
						geoWgts[c], 
						cetX, cetY, cetZ,
						geoMins[x], geoMins[y], geoMins[z],
						geoMaxs[x], geoMaxs[y], geoMaxs[z]
				);
			}
			fclose(fp);
