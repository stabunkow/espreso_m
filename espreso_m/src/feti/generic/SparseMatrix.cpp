...
#ifdef USE_DEVICE
void SparseMatrix::initDeviceMem()
{
	hipMalloc(&A_r, rows * cols * sizeof(double));
	hipMalloc(&x_r, cols * sizeof(double));
	hipMalloc(&y_r, rows * sizeof(double));
	hipHostMalloc(&A_h, rows * cols * sizeof(double), hipHostMallocDefault);
    hipHostMalloc(&x_h, cols * sizeof(double), hipHostMallocDefault);
    hipHostMalloc(&y_h, rows * sizeof(double), hipHostMallocDefault);
	if (type == 'S') 
	{
		int cnt = 0;
		for (int i = 0; i < rows; ++i) {
			for (int j = 0; j <= i; ++j) {
				A_h[i * rows + j] = dense_values[cnt++];
				A_h[j * rows + i] = A_h[i * rows + j];
			}
		}
	}
	else
	{
		memcpy(A_h, dense_values.data(), rows * cols * sizeof(double));
	}
	hipMemcpy(A_r, A_h, rows * cols * sizeof(double), hipMemcpyHostToDevice);
	dense_values = std::vector<double>();
}
void SparseMatrix::DenseMatVecDCU(rocblas_handle handle_r) 
{
	hipMemcpy(x_r, x_h, cols * sizeof(double), hipMemcpyHostToDevice);
	rocblas_operation trans_r;
	int lda_r = rows;
	trans_r = rocblas_operation_none;
	double alpha = 1.0, beta = 0;
	rocblas_status status_r = rocblas_dgemv(handle_r, trans_r, 
		rows, cols,
		&alpha, A_r, lda_r, 
		x_r, 1, 
		&beta, y_r, 1);
	if (status_r) {
		// printf("dgemv error:rank: %03d, deviceId: %d, handle：%p\n", info::mpi::rank, deviceId, handle_r);
		eslog::error("dgemv error %d\n", status_r);
		exit(EXIT_FAILURE);
	}
	hipMemcpy(y_h, y_r, rows * sizeof(double), hipMemcpyDeviceToHost);
}
void SparseMatrix::DenseMatVecDCUAsync(hipStream_t stream_r, rocblas_handle handle_r)
{
	hipMemcpyAsync(x_r, x_h, cols * sizeof(double), hipMemcpyHostToDevice, stream_r);
	rocblas_operation trans_r;
	int lda_r = rows;
	trans_r = rocblas_operation_none;
	double alpha = 1.0, beta = 0;
	...
	hipMemcpyAsync(y_h, y_r, rows * sizeof(double), hipMemcpyDeviceToHost, stream_r);
}
void SparseMatrix::freeDeviceMem()
{
	hipFree(A_r);
	hipFree(x_r);
	hipFree(y_r);
...
#endif
void SparseMatrix::initAlignedMem() {
	A_h = (double*) mkl_malloc(dense_values.size() * sizeof(double), 64);
    x_h = (double*) mkl_malloc(cols * sizeof(double), 64);
    y_h = (double*) mkl_malloc(rows * sizeof(double), 64);
	memcpy(A_h, dense_values.data(), dense_values.size() * sizeof(double));
	vector<double> tmp;
	dense_values.swap(tmp);
}
void SparseMatrix::copyInMem(double* x_in)
{
	memcpy(x_h, x_in, cols * sizeof(double));
}
void SparseMatrix::copyBackMem(double* y_out)
{
	memcpy(y_out, y_h, rows * sizeof(double));
}
void SparseMatrix::freeAlignedMem() 
{
	mkl_free(A_h);
	mkl_free(x_h);
	mkl_free(y_h);
}
double SparseMatrix::CountMemory(){
	double memory = 0;
	if (type == 'G') {
		double numOfDouble = rows * cols * 1.;
		double GBunit = 1024.0 * 1024.0 * 1024.0;
		memory = numOfDouble * sizeof(double) / GBunit;
	} else {
		double numOfDouble = rows * (rows + 1) / 2;
		double GBunit = 1024.0 * 1024.0 * 1024.0;
		memory = numOfDouble * sizeof(double) / GBunit;
	}
	return memory;
}
void SparseMatrix::DenseMatVecCPU() {
	// void cblas_dgemv
...
void SparseMatrix::getSubDiagBlockmatrix(SparseMatrix & A_in, SparseMatrix & A_out, esint i_start, esint size_rr){
#ifdef GENINVtools
//
...
//      esint ik=0,cnt_i=0;
//      ose3 << "permut_vector\n";
//      for (esint i = 0;i<permVec.size();i++){
//        ose3 << permVec[i] + 1<<" ";
//      }
//      ose3 << "\nnull_pivots\n";
//      for (esint i = 0; i < null_pivots.size(); i++){
...
//  os << "null pivots: \n";
//  os << "[";
//  for (esint k = 0; k<null_pivotsl.size();k++){
//    os << null_pivotsl[k] ;
