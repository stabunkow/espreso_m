...
public:
#ifdef USE_DEVICE
double *A_r, *x_r, *y_r; // device mem
void initDeviceMem();
void DenseMatVecDCU(rocblas_handle handle_r);
void DenseMatVecDCUAsync(hipStream_t stream_r, rocblas_handle handle_r);
void freeDeviceMem();
#endif
	double *A_h, *x_h, *y_h; // host aligned mem or pinned mem
	double CountMemory();
	void initAlignedMem();
	void copyInMem(double* x_in);
	void DenseMatVecCPU();
	void copyBackMem(double* y_out);
	void freeAlignedMem();
