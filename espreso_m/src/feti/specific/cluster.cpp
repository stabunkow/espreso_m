...
void ClusterBase::CreateF0() {
	 ...
	 if (defprint) { F0_timing.totalTime.start(); }
	...
	// eslog::checkpointln("HFETI - Create F0");
	 ...
	 if (defprint) { solve_F0_time.start(); }
...
//	//ESINFO(PROGRESS3);
	...
	if (defprint) { solve_F0_time.end(); }
	if (defprint) { solve_F0_time.printStatMPI(); }
	if (defprint) {F0_timing.addEvent(solve_F0_time);}

	TimeEvent reduction_F0_time("F0 reduction time");
	if (defprint) {reduction_F0_time.start();}
...
//	}
	...
	if (defprint) { reduction_F0_time.end(); reduction_F0_time.printStatMPI(); F0_timing.addEvent(reduction_F0_time); }


	TimeEvent fact_F0_time("B0 Kplus Factorization ");
	if (defprint) { fact_F0_time.start();}
...
//		F0_fast.ImportMatrix(F0_Mat);
	...
	if (defprint) {fact_F0_time.end(); fact_F0_time.printStatMPI(); F0_timing.addEvent(fact_F0_time);}

	if (defprint) { F0_timing.totalTime.end(); }
...
void ClusterBase::CreateSa() {
	 ...
	 TimeEval Sa_timing (" HFETI - Salfa preprocessing timing"); if (defprint) { Sa_timing.totalTime.start(); }
...
//	bool get_kernel_from_mesh = configuration.regularization == FETIConfiguration::REGULARIZATION::ANALYTIC;
	...
	SparseMatrix Salfa, tmpM;
	 ...
	 TimeEvent G0trans_Sa_time("G0 transpose"); if (defprint) { G0trans_Sa_time.start(); }
	SparseMatrix G0t;
	SparseMatrix G02t;
	G0.MatTranspose(G0t);
	if (!SYMMETRIC_SYSTEM) {
	 ...
	 if (defprint) { G0trans_Sa_time.end(); G0trans_Sa_time.printStatMPI(); Sa_timing.addEvent(G0trans_Sa_time); }


	 TimeEvent G0solve_Sa_time("SolveMatF with G0t as InitialCondition"); if (defprint) { G0solve_Sa_time.start(); }
	...
	if (SYMMETRIC_SYSTEM) {
	 ...
	 if (defprint) { G0solve_Sa_time.end(); G0solve_Sa_time.printStatMPI(); Sa_timing.addEvent(G0solve_Sa_time); }
...
//				}
	...
	if ( configuration.regularization == FETIConfiguration::REGULARIZATION::ANALYTIC ) {
		...
		// TODO: GENERALIZE
		 ...
		 TimeEvent reg_Sa_time("Salfa regularization "); if (defprint) { reg_Sa_time.start(); }
		...
		Salfa.mtype = this->mtype;
		 ...
		 if (defprint) { reg_Sa_time.end(); reg_Sa_time.printStatMPI(); Sa_timing.addEvent(reg_Sa_time); }
...
//	}
	...
	switch (configuration.SAsolver) {
		...
		case FETIConfiguration::SASOLVER::CPU_DENSE: {
			...
			if (defprint) { factd_Sa_time.start(); }
			...
			if (defprint) { factd_Sa_time.end(); }//factd_Sa_time.printStatMPI();
			if (defprint) { Sa_timing.addEvent(factd_Sa_time); }
			break;
		}

		case FETIConfiguration::SASOLVER::CPU_SPARSE: {
			TimeEvent fact_Sa_time("Salfa factorization ");
			if (defprint) { fact_Sa_time.start(); }
			...
			if (defprint) { fact_Sa_time.end(); } //fact_Sa_time.printStatMPI(); }
			if (defprint) { Sa_timing.addEvent(fact_Sa_time); }
			break;
		}
	...
	if (defprint) { Sa_timing.totalTime.end(); }
...
void ClusterBase::Create_G_perCluster() {
	...
	if (defprint) { G1_1_time.start(); }
	TimeEvent G1_1_mem  ("Create G1 per clust mem: MatMat+MatTrans ");
	if (defprint) { G1_1_mem.startWithoutBarrier(GetProcessMemory_u()); }
	...
	if (defprint) { G1_2_time.start(); }
	TimeEvent G1_2_mem  ("Create G1 per clust mem: Par.red.+MatAdd ");
	if (defprint) { G1_2_mem.startWithoutBarrier(GetProcessMemory_u()); }
