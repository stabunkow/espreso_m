...
struct ClusterInfo{
	int rank, curDomainID;
	vector<double> DomainRAM;
	bool isHelper;
	double clusterSCmem, curScMem;
	vector<int> commRank, commDomainBegin, commDomainNum;
};
struct MVInfo {
	SparseMatrix* B1Kplus;
	double* compressedTmp2;
	double* compressedTmp;
	double weight;
};
extern vector<ClusterInfo> allClusterInfo;
extern vector<double> allDomainRAM, myDomainRAM;
extern vector<int> allDomainSize, domainDispArr;
extern int myDomainSize, totalDomainSize;
extern double clusterSCmemAvg;
extern ClusterInfo myClusterInfo;
extern vector<MVInfo> mvjobs;
extern vector<MVInfo> premvjobs;
//class SuperCluster;
...
public:
	...
	std::vector<std::vector<int> > threadSc, threadPre;
#ifdef USE_DEVICE
	std::vector<rocblas_handle> deviceHandles;
	std::vector<std::vector<hipStream_t> > deviceStreams;
	std::vector<std::vector<rocblas_handle> > deviceStreamHandles;
#endif
	// *** Variables *************
	...
	int Solve     ( SuperCluster & cluster, SEQ_VECTOR < SEQ_VECTOR <double> > & in_right_hand_side_primal, SEQ_VECTOR < SEQ_VECTOR <double> > & out_primal_solution_parallel, SEQ_VECTOR < SEQ_VECTOR <double> > & out_dual_solution_parallel );
