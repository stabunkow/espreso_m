...
namespace espreso{
	...
	vector<double> allDomainRAM, myDomainRAM;
	vector<int> allDomainSize, domainDispArr;
	int myDomainSize = 0, totalDomainSize = 0;
	double clusterSCmemAvg = 0.0;
	ClusterInfo myClusterInfo;
	char SC_type, Pre_type;
	vector<MVInfo> mvjobs;
	vector<MVInfo> premvjobs;
	bool clusterInfoCmp(ClusterInfo &A, ClusterInfo &B){
		return A.clusterSCmem < B.clusterSCmem;
	}
	void collectInformation(SuperCluster & cluster, double countMem_sc){
		myDomainSize = cluster.domains.size();
		myDomainRAM.resize(cluster.domains.size());
		allDomainSize.resize(info::mpi::size);
		for(size_t d = 0; d < cluster.domains.size(); d++)
			myDomainRAM[d] = cluster.domains[d]->B1Kplus.CountMemory();
		MPI_Allgather(&myDomainSize, 1, MPI_INT, &allDomainSize[0], 1, MPI_INT, info::mpi::comm);
		for(int x : allDomainSize)
			totalDomainSize += x;
		allDomainRAM.resize(totalDomainSize);
		domainDispArr.resize(info::mpi::size);
		for(int i = 1; i < info::mpi::size; i++){
			domainDispArr[i] = domainDispArr[i - 1] + allDomainSize[i - 1]; 
		}
		MPI_Allgatherv(&myDomainRAM[0], myDomainSize, MPI_DOUBLE, &allDomainRAM[0], &allDomainSize[0], &domainDispArr[0], MPI_DOUBLE, info::mpi::comm);
		MPI_Allreduce(&countMem_sc, &clusterSCmemAvg, 1, MPI_DOUBLE, MPI_SUM, info::mpi::comm);
		clusterSCmemAvg /= (double)info::mpi::size;
		allClusterInfo.resize(info::mpi::size);
		for(int i = 0; i < info::mpi::size; i++){
			allClusterInfo[i].rank = i;
			allClusterInfo[i].DomainRAM.resize(allDomainSize[i]);
			allClusterInfo[i].clusterSCmem = 0.0;
			allClusterInfo[i].curDomainID = 0;
			for(int j = 0; j < allDomainSize[i]; j++){
				allClusterInfo[i].DomainRAM[j] = allDomainRAM[j + domainDispArr[i]];
				allClusterInfo[i].clusterSCmem += allDomainRAM[j + domainDispArr[i]];
			}
			if(allClusterInfo[i].clusterSCmem < clusterSCmemAvg)
				allClusterInfo[i].isHelper = true;
			else allClusterInfo[i].isHelper = false;
			allClusterInfo[i].curScMem = allClusterInfo[i].clusterSCmem;
		}
		std::sort(allClusterInfo.begin(), allClusterInfo.end(), clusterInfoCmp);
	}
	void loadBalanceLocal(){
		auto helper = allClusterInfo.begin();
		auto helpee = allClusterInfo.end() - 1;
		while (helpee->isHelper == false && helper->isHelper) {
			helpee->commRank.push_back(helper->rank);
			helper->commRank.push_back(helpee->rank);
			helpee->commDomainBegin.push_back(helpee->curDomainID);
			int cntDomain = 0;
			while (helpee->curScMem > clusterSCmemAvg && helper->curScMem < clusterSCmemAvg) {
				helpee->curScMem -= helpee->DomainRAM[helpee->curDomainID];
				helper->curScMem += helpee->DomainRAM[helpee->curDomainID++];
				cntDomain++;
			}
			helpee->commDomainNum.push_back(cntDomain);
			helper->commDomainNum.push_back(cntDomain);
			if (helpee->curScMem <= clusterSCmemAvg) helpee--;
			if (helper->curScMem >= clusterSCmemAvg) helper++;
		}
		for(auto oneClusterInfo : allClusterInfo){
			if(info::mpi::rank == oneClusterInfo.rank) myClusterInfo = oneClusterInfo;
		}
		if(info::mpi::rank == 0){
			printf("allClusterInfo\n");
			for(auto oneClusterInfo : allClusterInfo){
				printf("rank = %d\n", oneClusterInfo.rank);
				printf("curScMem = %lf\n", oneClusterInfo.curScMem);
				printf("clusterSCmem = %lf\n", oneClusterInfo.clusterSCmem);
				printf("commRank = {");
				bool isP = false;
				for(int x : oneClusterInfo.commRank){
					if(isP) printf(", %d", x);
					else{
						printf("%d", x);
						isP = true;
				...
				printf("}\n");
				printf("commDomainNum = {");
				isP = false;
				for(int x : oneClusterInfo.commDomainNum){
					if(isP) printf(", %d", x);
					else{
						printf("%d", x);
						isP = true;
				...
				printf("}\n");
				printf("commDomainBegin = {");
				isP = false;
				for(int x : oneClusterInfo.commDomainBegin){
					if(isP) printf(", %d", x);
					else{
						printf("%d", x);
						isP = true;
	...
	void loadBalanceByMPI(SuperCluster & cluster){
		if(myClusterInfo.isHelper){
			cluster.helpSiz = 0;
			for(size_t i = 0; i < myClusterInfo.commRank.size(); i++) cluster.helpSiz += myClusterInfo.commDomainNum[i];
			cluster.helpM.resize(cluster.helpSiz);
			cluster.recvCompressedTmp2.resize(cluster.helpSiz);
			cluster.sendCompressedTmp.resize(cluster.helpSiz);
			cluster.helpG.resize(cluster.helpSiz);
			cluster.recvxPrimCluster1.resize(cluster.helpSiz);
			cluster.sendxPrimCluster2.resize(cluster.helpSiz);
			cluster.request_recv1.resize(cluster.helpSiz);
			cluster.request_send2.resize(cluster.helpSiz);
			for (int i = 0; i < cluster.helpSiz; ++i) {
				cluster.helpM[i].USE_FLOAT = false;
				if (SC_type == 'S') {
					cluster.helpM[i].type = 'S';
				} else {
					cluster.helpM[i].type = 'G';
				}
				cluster.helpG[i].USE_FLOAT = false;
				if (Pre_type == 'S') {
					cluster.helpG[i].type = 'S';
				} else {
					cluster.helpG[i].type = 'G';
				}
			}
			...
			for(size_t i = 0; i < myClusterInfo.commRank.size(); i++) {
				for(int j = 0; j < myClusterInfo.commDomainNum[i]; j++) {
					int recvRows = 0;
					MPI_Recv(&recvRows, 1, MPI_INT, myClusterInfo.commRank[i], 1003 + j, info::mpi::comm, MPI_STATUSES_IGNORE);
					cluster.helpM[hcnt].rows = recvRows;
					cluster.helpM[hcnt].cols = recvRows;
					cluster.recvCompressedTmp2[hcnt].resize(recvRows);
					cluster.sendCompressedTmp[hcnt].resize(recvRows);
					int recvDenseNum;
					if (SC_type == 'S') {
						recvDenseNum = (recvRows + 1) * recvRows / 2;
					} else {
						recvDenseNum = recvRows * recvRows;
					}
					cluster.helpM[hcnt].dense_values.resize(recvDenseNum);
					// printf("helper-%03d: %03d, helpM.rows: %d, helpM.cols: %d, ctmp2: %d, ctmp: %d, densesize: %d\n",
					// 	info::mpi::rank, hcnt, cluster.helpM[hcnt].rows, cluster.helpM[hcnt].cols,
					// 		cluster.recvCompressedTmp2[hcnt].size(), cluster.sendCompressedTmp[hcnt].size(),
					// 		cluster.helpM[hcnt].dense_values.size());
					MPI_Recv(&cluster.helpM[hcnt].dense_values[0], recvDenseNum, MPI_DOUBLE, myClusterInfo.commRank[i], 10000 + j, info::mpi::comm, MPI_STATUSES_IGNORE);
					hcnt++;
				}
			}
			Communication::barrier();
			...
			for(size_t i = 0; i < myClusterInfo.commRank.size(); i++){
				for(int j = 0; j < myClusterInfo.commDomainNum[i]; j++){
					int recvRows = 0;
					MPI_Recv(&recvRows, 1, MPI_INT, myClusterInfo.commRank[i], 50000 + j, info::mpi::comm, MPI_STATUSES_IGNORE);
					cluster.helpG[hcnt].rows = recvRows;
					cluster.helpG[hcnt].cols = recvRows;
					cluster.recvxPrimCluster1[hcnt].resize(recvRows);
					cluster.sendxPrimCluster2[hcnt].resize(recvRows);
          			int recvDenseNum;
					if (Pre_type == 'S') {
						recvDenseNum = (recvRows + 1)*recvRows/2;
					} else {
          			...
          			cluster.helpG[hcnt].dense_values.resize(recvDenseNum);
					// printf("helper-%03d: %03d, helpG.rows: %d, helpG.cols: %d, prim1: %d, prim2: %d, densesize: %d\n",
					// 	info::mpi::rank, hcnt, cluster.helpG[hcnt].rows, cluster.helpG[hcnt].cols,
					// 		cluster.recvxPrimCluster1[hcnt].size(), cluster.sendxPrimCluster2[hcnt].size(),
					// 		cluster.helpG[hcnt].dense_values.size());
          			MPI_Recv(&cluster.helpG[hcnt].dense_values[0], recvDenseNum, MPI_DOUBLE, myClusterInfo.commRank[i], 60000 + j, info::mpi::comm, MPI_STATUSES_IGNORE);
          			hcnt++;
				}
			}
		}
			...
			cluster.helpSiz = 0;
      		for(size_t i = 0; i < myClusterInfo.commRank.size(); i++) cluster.helpSiz += myClusterInfo.commDomainNum[i];
			cluster.request_send1.resize(cluster.helpSiz);
			cluster.request_recv2.resize(cluster.helpSiz);

			for(size_t i = 0; i < myClusterInfo.commRank.size(); i++){
				for(int j = 0; j < myClusterInfo.commDomainNum[i]; j++){
					int domainId = j + myClusterInfo.commDomainBegin[i];
					int sendRows = cluster.domains[domainId]->B1Kplus.rows;
					MPI_Send(&sendRows, 1, MPI_INT, myClusterInfo.commRank[i], 1003 + j, info::mpi::comm);
          			int sendDenseNum;
					if (SC_type == 'S') {
						sendDenseNum = (sendRows + 1) * sendRows / 2;
					} else {
						sendDenseNum = sendRows * sendRows;
					}
          			MPI_Send(&cluster.domains[domainId]->B1Kplus.dense_values[0], sendDenseNum, MPI_DOUBLE, myClusterInfo.commRank[i], 10000 + j, info::mpi::comm);
				}
			}
			Communication::barrier();
			...
			for(size_t i = 0; i < myClusterInfo.commRank.size(); i++){
				for(int j = 0; j < myClusterInfo.commDomainNum[i]; j++){
					int domainId = j + myClusterInfo.commDomainBegin[i];
					int sendRows = cluster.domains[domainId]->Prec.rows;
					MPI_Send(&sendRows, 1, MPI_INT, myClusterInfo.commRank[i], 50000 + j, info::mpi::comm);
          			int sendDenseNum;
					if (Pre_type == 'S') {
						sendDenseNum = (sendRows + 1)*sendRows/2;
					} else {
          			...
          			MPI_Send(&cluster.domains[domainId]->Prec.dense_values[0], sendDenseNum, MPI_DOUBLE, myClusterInfo.commRank[i], 60000 + j, info::mpi::comm);
				}
			}
		}

		if(info::mpi::rank == 0)
			printf("LoadBalanceByMPI successfully!\n");
		Communication::barrier();
	}
	void printMPIBalanceMem(SuperCluster & cluster, double memSc, double memG)
	{
		if (myClusterInfo.isHelper) {
			for(size_t i = 0; i < cluster.helpSiz; i++) {
				memSc += cluster.helpM[i].CountMemory();
				memG += cluster.helpG[i].CountMemory();
			}
		} else {
			for(size_t i = 0; i < myClusterInfo.commRank.size(); i++){
				for(int j = 0; j < myClusterInfo.commDomainNum[i]; j++){
					int domainId = j + myClusterInfo.commDomainBegin[i];
					double mem1 = cluster.domains[domainId]->B1Kplus.CountMemory();
					double mem2 = cluster.domains[domainId]->Prec.CountMemory();
					memSc -= mem1;
					memG -= mem2;
				}
			}
		}
		info::mpi::printStatsMPI("After MPI balance LSC_Memory", memSc);
		info::mpi::printStatsMPI("After MPI balance Pre_Memory", memG);
	}
	void printLSCStat(SuperCluster& cluster, double& countMem_sc, double& countMem_pre)
	{
		// stat LSC Memeroy
		// std::vector<double> domainSCSizs(cluster.domains.size());
		// std::vector<double> domainPreSizs(cluster.domains.size());
		for (size_t d = 0; d < cluster.domains.size(); ++d) {
			double size = cluster.domains[d]->B1Kplus.CountMemory();
			// domainSCSizs[d] = size;
			countMem_sc += size;
		}
		SC_type = cluster.domains[0]->B1Kplus.type;
		if (info::mpi::rank == 0) {
			printf("LSC type: %c\n", cluster.domains[0]->B1Kplus.type);
		}
		info::mpi::printStatsMPI("LSC_Memory", countMem_sc);

		for (size_t d = 0; d < cluster.domains.size(); ++d) {
			double size = cluster.domains[d]->Prec.CountMemory();
			// domainPreSizs[d] = size;
			countMem_pre += size;
		}
		Pre_type = cluster.domains[0]->Prec.type;
		if (info::mpi::rank == 0) {
			printf("Prec type: %c\n", cluster.domains[0]->Prec.type);
		}
		info::mpi::printStatsMPI("Pre_Memory", countMem_pre);

		info::mpi::printStatsMPI("lambdas_local_size", cluster.my_lamdas_indices.size());
	}
	void saveMat(SuperCluster& cluster)
	{
		char filename[30];
		int a1 = info::mpi::size;
		int a2 = info::ecf->input.decomposition.domains;
		int a3 = info::ecf->input.loaddomain;
		int m1 = info::mpi::rank;
		sprintf(filename, "./save_mat/save_mat_%04d_%04d_%d/mat_%04d", a1, a2, a3, m1);
		FILE *fp;
		fp = fopen(filename, "w+");
		fprintf(fp, "%d\n", cluster.domains.size());
		for (int d = 0; d < cluster.domains.size(); ++d) {
			int rows = cluster.domains[d]->B1Kplus.rows;
			fprintf(fp, "%d\n", rows);
		}
		fclose(fp);
		Communication::barrier();
		...
		if (info::ecf->input.use_mpi_balance) {
			sprintf(filename,  "./save_mat/save_mat_%04d_%04d_%d/hmat_%04d", a1, a2, a3, m1);
			fp = fopen(filename, "w+");
			fprintf(fp, "%d\n", myClusterInfo.isHelper);
			if (myClusterInfo.isHelper) {
				fprintf(fp, "%d\n", myClusterInfo.commRank.size());
				int hcnt = 0;
				for (int i = 0; i < myClusterInfo.commRank.size(); ++i) {
					fprintf(fp, "%d", myClusterInfo.commDomainNum[i]);
					for (int d = 0; d < myClusterInfo.commDomainNum[i]; ++d) {
						fprintf(fp, " %d", cluster.helpM[hcnt].rows);
						hcnt++;
					}
					fprintf(fp, "\n");
				}
			} else {
				fprintf(fp, "%d\n", myClusterInfo.curDomainID);
			}
			fclose(fp);
			Communication::barrier();
		}
		if (info::mpi::rank == 0) printf("mat saved.\n");
		Communication::barrier();
	}
}
...
{
	double countMem_sc = 0, countMem_pre = 0;
	printLSCStat(cluster, countMem_sc, countMem_pre);
	if (USE_PREC == FETIConfiguration::PRECONDITIONER::DIRICHLET && USE_KINV)
	{
		if (info::ecf->input.use_mpi_balance) 
		{
			collectInformation(cluster, countMem_sc);
			loadBalanceLocal();
			loadBalanceByMPI(cluster);
			printMPIBalanceMem(cluster, countMem_sc, countMem_pre);
		}
		if (info::ecf->input.save_mat) saveMat(cluster);
		
		if (info::ecf->input.use_mpi_balance)
		{
			if (myClusterInfo.isHelper)
			{
				int workSiz = cluster.domains.size() + cluster.helpSiz;
				mvjobs.resize(workSiz);
				premvjobs.resize(workSiz);
				for (int i = 0; i < cluster.domains.size(); ++i)
				{
					mvjobs[i].B1Kplus = &cluster.domains[i]->B1Kplus;
					mvjobs[i].compressedTmp2 = cluster.domains[i]->compressed_tmp2.data();
					mvjobs[i].compressedTmp = cluster.domains[i]->compressed_tmp.data();
					mvjobs[i].weight = cluster.domains[i]->B1Kplus.CountMemory();

					premvjobs[i].B1Kplus = &cluster.domains[i]->Prec;
					premvjobs[i].compressedTmp2 = cluster.x_prim_cluster1[i]->data();
					premvjobs[i].compressedTmp = cluster.x_prim_cluster2[i]->data();
					premvjobs[i].weight = cluster.domains[i]->Prec.CountMemory();
				}
				int domainSiz = cluster.domains.size();
				int hcnt = 0;
				for (int i = 0; i < myClusterInfo.commRank.size(); ++i) {
					for (int d = 0; d < myClusterInfo.commDomainNum[i]; ++d) {
						mvjobs[domainSiz + hcnt].B1Kplus = &cluster.helpM[hcnt];
						mvjobs[domainSiz + hcnt].compressedTmp2 = cluster.recvCompressedTmp2[hcnt].data();
						mvjobs[domainSiz + hcnt].compressedTmp = cluster.sendCompressedTmp[hcnt].data();
						mvjobs[domainSiz + hcnt].weight = cluster.helpM[hcnt].CountMemory();

						premvjobs[domainSiz + hcnt].B1Kplus = &cluster.helpG[hcnt];
						premvjobs[domainSiz + hcnt].compressedTmp2 = cluster.recvxPrimCluster1[hcnt].data();
						premvjobs[domainSiz + hcnt].compressedTmp = cluster.sendxPrimCluster2[hcnt].data();
						premvjobs[domainSiz + hcnt].weight = cluster.helpG[hcnt].CountMemory();
						hcnt++;
					}
				}
			}
			...
			{
				int workSiz = cluster.domains.size() - myClusterInfo.curDomainID;
				mvjobs.resize(workSiz);
				premvjobs.resize(workSiz);
				for (int i = 0; i < workSiz; ++i)
				{
					mvjobs[i].B1Kplus = &cluster.domains[myClusterInfo.curDomainID + i]->B1Kplus;
					mvjobs[i].compressedTmp2 = cluster.domains[myClusterInfo.curDomainID + i]->compressed_tmp2.data();
					mvjobs[i].compressedTmp = cluster.domains[myClusterInfo.curDomainID + i]->compressed_tmp.data();
					mvjobs[i].weight = cluster.domains[myClusterInfo.curDomainID + i]->B1Kplus.CountMemory();

					premvjobs[i].B1Kplus = &cluster.domains[myClusterInfo.curDomainID + i]->Prec;
					premvjobs[i].compressedTmp2 = cluster.x_prim_cluster1[myClusterInfo.curDomainID + i]->data();
					premvjobs[i].compressedTmp = cluster.x_prim_cluster2[myClusterInfo.curDomainID + i]->data();
					premvjobs[i].weight = cluster.domains[myClusterInfo.curDomainID + i]->Prec.CountMemory();
				}
			}
		}
		else
		{
			int workSiz = cluster.domains.size();
			mvjobs.resize(workSiz);
			premvjobs.resize(workSiz);
			for (int i = 0; i < workSiz; ++i)
			{
		...
		if (info::ecf->input.use_device) {
#ifdef USE_DEVICE
			if (info::ecf->input.use_device_async == 0)
			{
			...
			{
				deviceStreams.resize(deviceCount);
				deviceStreamHandles.resize(deviceCount);
				for (int deviceId = 0; deviceId < deviceCount; ++deviceId)
				{
					hipSetDevice(deviceId);
					deviceStreams[deviceId].resize(deviceStreamCount);
					deviceStreamHandles[deviceId].resize(deviceStreamCount);
					for (int s = 0; s < deviceStreamCount; ++s) {
						hipStreamCreate(&deviceStreams[deviceId][s]);
			...
			std::vector<double> D0(deviceCount, 1);
			std::vector<double> P1(mvjobs.size()), P2(premvjobs.size());
			for (int i = 0; i < mvjobs.size(); ++i) P1[i] = mvjobs[i].weight;
			for (int i = 0; i < premvjobs.size(); ++i) P2[i] = premvjobs[i].weight;
			processMapping(D0, P1, threadSc);
			processMapping(D0, P2, threadPre);

			for (int deviceId = 0; deviceId < deviceCount; ++deviceId)
			{
				hipSetDevice(deviceId);
				for (int i = 0; i < threadSc[deviceId].size(); ++i) {
					int d = threadSc[deviceId][i];
					// mvjobs[d].B1Kplus->initPinnedMem();
					mvjobs[d].B1Kplus->initDeviceMem();
				}
			}
			...
			{
				hipSetDevice(deviceId);
				for (int i = 0; i < threadPre[deviceId].size(); ++i) {
					int d = threadPre[deviceId][i];
					// premvjobs[i].B1Kplus->initPinnedMem();
					premvjobs[d].B1Kplus->initDeviceMem();
				}
			}

			if (info::ecf->input.use_device_print) {
				char filename[30];
				FILE *fp;
				sprintf(filename, "./device_info/rank_%04d", info::mpi::rank);
				fp = fopen(filename, "w+");
				fprintf(fp, "domains: %d, %.6f\n", cluster.domains.size(), countMem_sc);
				for (int deviceId = 0; deviceId < deviceCount; ++deviceId) {
					double mem = 0;
					for (int i = 0; i < threadSc[deviceId].size(); ++i) {
						int d = threadSc[deviceId][i];
					...
					fprintf(fp, "device %d: %d, %.6f\n", deviceId, threadSc[deviceId].size(), mem);
				}
				fclose(fp);
			}
#endif
		...
		{
			for (int i = 0; i < mvjobs.size(); ++i) {
				mvjobs[i].B1Kplus->initAlignedMem();
			}
			for (int i = 0; i < premvjobs.size(); ++i) {
				premvjobs[i].B1Kplus->initAlignedMem();
			}
		}
	}
	...
	if (USE_PREC == FETIConfiguration::PRECONDITIONER::DIRICHLET && USE_KINV)
	{
		...
		if (info::ecf->input.use_device) {
			...
			for (int deviceId = 0; deviceId < deviceCount; ++deviceId)
			{
				...
				for (int i = 0; i < threadSc[deviceId].size(); ++i) {
					int d = threadSc[deviceId][i];
					mvjobs[d].B1Kplus->freeDeviceMem();
					// mvjobs[d].B1Kplus->freePinnedMem();
				}
			}
			for (int deviceId = 0; deviceId < deviceCount; ++deviceId)
			{
				...
				for (int i = 0; i < threadPre[deviceId].size(); ++i) {
					int d = threadPre[deviceId][i];
					premvjobs[d].B1Kplus->freeDeviceMem();
					// premvjobs[d].B1Kplus->freePinnedMem();
				}
			}
			...
			if (info::ecf->input.use_device_async == 0)
			{
				...
				{
					hipSetDevice(deviceId);
					rocblas_destroy_handle(deviceHandles[deviceId]);
				}
			}
			else
			{
				for (int deviceId = 0; deviceId < deviceCount; ++deviceId)
				{
					hipSetDevice(deviceId);
					for (int s = 0; s < deviceStreamCount; ++s) {
						rocblas_destroy_handle(deviceStreamHandles[deviceId][s]);
						hipStreamDestroy(deviceStreams[deviceId][s]);
					}
				}
			}
#endif
		...
		{
			for (int i = 0; i < mvjobs.size(); ++i) {
				mvjobs[i].B1Kplus->freeAlignedMem();
			}
			for (int i = 0; i < premvjobs.size(); ++i) {
				premvjobs[i].B1Kplus->freeAlignedMem();
			}
		}
	}
	return iters;
...
for (size_t i = 0; i < y_l.size(); i++)
			...
			ddot_beta.start();
			if (info::ecf->input.use_ddot_opt == 1) {
				beta_l = parallel_ddot_compressed_opt(cluster, y_l, w_l, yp_l, wp_l);
			} else if (info::ecf->input.use_ddot_opt == 2) {
				beta_l =          parallel_ddot_compressed_opt2(cluster, y_l, w_l);
				beta_l = beta_l / parallel_ddot_compressed_opt2(cluster, yp_l, wp_l);
			} else {
...
for (size_t i = 0; i < p_l.size(); i++)
		...
		//------------------------------------------
		 ddot_alpha.start();
		if (info::ecf->input.use_ddot_opt == 1) {
			alpha_l = parallel_ddot_compressed_opt(cluster, y_l, w_l, p_l, Ap_l);
		} else if (info::ecf->input.use_ddot_opt == 2) {
			alpha_l =           parallel_ddot_compressed_opt2(cluster, y_l, w_l);
			alpha_l = alpha_l / parallel_ddot_compressed_opt2(cluster, p_l, Ap_l);
		} else {
...
//			<< indent << std::fixed << std::setprecision(5) << timing.totalTime.getLastStat();
	...
	timeEvalAppa.timeEvents[0].eventCount--;
	timeEvalAppa.timeEvents[0].eventTime.erase(timeEvalAppa.timeEvents[0].eventTime.begin());
	timeEvalAppa.timeEvents[0].evaluate();
	timeEvalAppa.timeEvents[1].eventCount--;
	timeEvalAppa.timeEvents[1].eventTime.erase(timeEvalAppa.timeEvents[1].eventTime.begin());
	timeEvalAppa.timeEvents[1].evaluate();
	timeEvalAppa.timeEvents[2].evaluate();
	timeEvalAppa.timeEvents[3].evaluate();
	info::mpi::printStatsMPI("apa_B1t.sumTime", timeEvalAppa.timeEvents[0].sumTime);
	info::mpi::printStatsMPI("apa_kplus.sumTime", timeEvalAppa.timeEvents[1].sumTime);
	info::mpi::printStatsMPI("apa_B1.sumTime", timeEvalAppa.timeEvents[2].sumTime);
	info::mpi::printStatsMPI("apa_allred.sumTime", timeEvalAppa.timeEvents[3].sumTime);

	if (info::mpi::rank == 0) {
		printf("use_ddot_opt: %d\n", info::ecf->input.use_ddot_opt);
	}
	ddot_beta.evaluate();
	ddot_alpha.evaluate();
	info::mpi::printStatsMPI("Beta_ddot.sumTime", ddot_beta.sumTime);
	info::mpi::printStatsMPI("Alpha_ddot.sumTime", ddot_alpha.sumTime);
...
double parallel_ddot_compressed_opt( SuperCluster & cluster, SEQ_VECTOR<double> & input_vector1, SEQ_VECTOR<double> & input_vector2, SEQ_VECTOR<double> & input_vector3, SEQ_VECTOR<double> & input_vector4){
	double a1 = 0; double a1g = 0;
	...
	double a2 = 0, a2g = 0;
	#pragma omp parallel for reduction(+:a2)
	for (int i = -1; i < cluster.my_lamdas_indices.size(); i++)  {
		if(i == -1){
			MPI_Allreduce( &a1, &a1g, 1, MPI_DOUBLE, MPI_SUM, info::mpi::comm);
		}
		else
			a2 = a2 + (input_vector3[i] * input_vector4[i] * cluster.my_lamdas_ddot_filter[i]);
	}

	MPI_Allreduce( &a2, &a2g, 1, MPI_DOUBLE, MPI_SUM, info::mpi::comm);

	return a1g/a2g;
}

double parallel_ddot_compressed_opt2( SuperCluster & cluster, SEQ_VECTOR<double> & input_vector1, SEQ_VECTOR<double> & input_vector2 )
{
	double a1 = 0; double a1g = 0;
